...
* `app_task` orchestrates the sub-tasks and provides linkage of shared data.
  ...
  * Run the `traxx_task` to exchange a packet with the server
...
* `traxx_task` opens a socket and exchanges a packet with the server.
...
* Capture NTP timestamp [implemented but buggy]
